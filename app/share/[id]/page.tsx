...
async function getPost(shareId: string) {
  // Intentar con columna copy_text primero, luego content
...
// Genera los meta tags OG que WhatsApp lee para el preview
export async function generateMetadata(
  ...
  if (!post) {
    return {
      title: 'SocialAI — Contenido compartido',
      description: 'Creá contenido para redes sociales con IA',
    };
  }
    ...
    return (
      <div style={{ minHeight: '100vh', background: '#FAF8F5', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
        <div style={{ textAlign: 'center', padding: 32 }}>
          ...
          <p style={{ color: '#7B6E99', marginBottom: 24 }}>Este link puede haber expirado o no existe.</p>
          <a href={appUrl} style={{ background: 'linear-gradient(135deg, #7C5CBF, #B07FE8)', color: '#fff', padding: '12px 24px', borderRadius: 12, textDecoration: 'none', fontWeight: 700 }}>
            Crear contenido con SocialAI
          </a>
        </div>
      </div>
    );
    ...
    <>
      ...
      <div style={{ minHeight: '100vh', background: '#FAF8F5', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
        ...
        <header style={{ background: '#fff', borderBottom: '1px solid #E8E0F0', padding: '14px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ fontFamily: "'Nunito', sans-serif", fontSize: 20, fontWeight: 800, background: 'linear-gradient(135deg, #7C5CBF, #B07FE8)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
            ✦ SocialAI
          </div>
          <a href={appUrl} style={{ background: 'linear-gradient(135deg, #7C5CBF, #B07FE8)', color: '#fff', padding: '8px 18px', borderRadius: 10, textDecoration: 'none', fontSize: 13, fontWeight: 700 }}>
            Crear contenido gratis
          </a>
        </header>
        ...
        <main style={{ maxWidth: 600, margin: '0 auto', padding: '32px 24px' }}>
          ...
          <div style={{ background: '#fff', border: '1.5px solid #E8E0F0', borderRadius: 18, padding: 24, marginBottom: 24, boxShadow: '0 2px 16px rgba(124,92,191,.06)' }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: '#B5AACC', textTransform: 'uppercase', letterSpacing: 1.2, marginBottom: 12 }}>
              ✦ Contenido generado con IA
            </div>
            <p style={{ fontSize: 15, lineHeight: 1.8, color: '#2D2640', whiteSpace: 'pre-wrap', margin: 0 }}>
              {copyText}
            </p>
          </div>
          ...
          <div style={{ background: 'linear-gradient(135deg, #EDE8F8, #FEF0E7)', border: '1.5px solid #E8E0F0', borderRadius: 18, padding: 24, textAlign: 'center' }}>
            <p style={{ fontSize: 14, color: '#7B6E99', marginBottom: 16, margin: '0 0 16px' }}>
              Creado con <strong style={{ color: '#7C5CBF' }}>SocialAI</strong> — Generá contenido para tus redes con IA
            </p>
            <a href={appUrl} style={{ background: 'linear-gradient(135deg, #7C5CBF, #B07FE8)', color: '#fff', padding: '13px 28px', borderRadius: 14, textDecoration: 'none', fontSize: 14, fontWeight: 700, display: 'inline-block', boxShadow: '0 4px 16px rgba(124,92,191,.30)' }}>
              🚀 Probalo gratis
            </a>
          </div>
